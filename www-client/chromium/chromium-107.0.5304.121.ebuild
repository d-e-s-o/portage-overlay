...
IUSE="+X +ungoogled component-build cups cpu_flags_arm_neon debug gtk4 +hangouts headless +js-type-check kerberos libcxx lto +official pgo pic +proprietary-codecs pulseaudio qt5 screencast selinux +suid +system-av1 +system-ffmpeg +system-harfbuzz +system-icu +system-png vaapi wayland widevine"
REQUIRED_USE="
...
chromium_configure() {
	...
	if use ungoogled; then
		...
		# (definitely the case for enable_one_click_signin).
