...
inherit python-any-no-clowntown qmake-utils readme.gentoo-r1 systemd toolchain-funcs virtualx xdg-utils

DESCRIPTION="Open-source version of Google Chrome web browser"
HOMEPAGE="https://www.chromium.org/"
PATCH_V="${PV%%\.*}"
SRC_URI="https://commondatastorage.googleapis.com/chromium-browser-official/${P}.tar.xz
	https://github.com/Eloston/ungoogled-chromium/archive/${PV}-1.tar.gz -> ungoogled-chromium-${PV}.tar.gz
	system-toolchain? (
...
IUSE="+X +ungoogled ${IUSE_SYSTEM_LIBS} bindist cups debug ffmpeg-chromium gtk4 +hangouts headless kerberos libcxx +lto +official pax-kernel pgo +proprietary-codecs pulseaudio"
...
src_unpack() {
	...
	mv ${WORKDIR}/ungoogled-chromium-${PV}-1 ${WORKDIR}/ungoogled-chromium-${PV} || die
}

src_prepare() {
	# The recipe is taken from ungoogled-chromium-portablelinux's [1]
	# but includes adjustments.
	# Note that we keep this step before python_setup because then
	# python3 is still implicitly referencable (instead of having to
	# hardcode /usr/bin/python3).
	# [1] https://github.com/ungoogled-software/ungoogled-chromium-portablelinux/blob/master/build.sh
	if use ungoogled; then
		_main_repo="${WORKDIR}/ungoogled-chromium-${PV}"
		_src_dir="${S}"

		einfo "Pruning binaries"

		# Some part of the build actually needs the esbuild binary, so
		# make sure not to prune it.
		sed -i '/esbuild/d' "$_main_repo/pruning.list" || die

		"$_main_repo/utils/prune_binaries.py" "$_src_dir" "$_main_repo/pruning.list" || die
		einfo "Applying patches"
		# TODO: ungoogled-chromium-portablelinux actually contains a
		#       bunch of patches itself. Should we include them?
		"$_main_repo/utils/patches.py" apply "$_src_dir" "$_main_repo/patches" || die
		einfo "Applying domain substitution"
		"$_main_repo/utils/domain_substitution.py" apply -r "$_main_repo/domain_regex.list" -f "$_main_repo/domain_substitution.list" -c "$_main_repo/domsubcache.tar.gz" "$_src_dir" || die
	fi
...
chromium_configure() {
	...
	if use ungoogled; then
		# Pick chosen flags from ungoogled-chromium-*/flags.gn.
		# Note that depending on how flags are used by the Chromium
		# infrastructure itself, the defaults in ungoogled-chromium's
		# flags.gn may not apply. You will have to set them here.
		# Also see //flags.gn and/or //chrome/common/features.gni
		# In general, search for `buildflags.gni` to find all configurable
		# features, I guess.
		# Commented out are option that seem useful to keep unnecessary
		# shit to a minimum, but that resulted in link errors in the
		# past.
		...
		myconf_gn+=" use_mpris=false"
		#myconf_gn+=" enable_background_contents=false"
		myconf_gn+=" enable_background_mode=false"
		...
		#myconf_gn+=" enable_discovery=false"
...
src_compile() {
	...
	strip out/Release/chromedriver || die
